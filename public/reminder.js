...
function validateMinutes(minutes) {
    // Check if it's a number between 10 and 120
    if (isNaN(minutes) || minutes < 10 || minutes > 120) {
        return false;
    }
    // Check if it's in 10-minute increments
    return minutes % 10 === 0;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (existingReminder) {
        ...
        if (now < existingReminder.endTime) {
            // Resume existing reminder
            startTimer(existingReminder.endTime - now);
    ...
    setReminderBtn.addEventListener('click', () => {
        ...
        if (!validateMinutes(minutes)) {
            alert('Please enter a valid number of minutes (10-120, in steps of 10)');
            return;
        }
